...
import { formatAIContextForPrompt } from '../contextFormatter';
...
function requireGrounding(raw: JsonObject): AiGrounding {
  const value = raw.grounding;
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new LlmError('invalid_output', 'AI出力に事実・仮説・未確認事項の根拠整理がありませんでした。');
  }
  const object = value as JsonObject;
  const confirmedFacts = strArray(object, 'confirmedFacts', []);
  const hypotheses = strArray(object, 'hypotheses', []);
  const unknowns = strArray(object, 'unknowns', []);
  const cautions = strArray(object, 'cautions', []);
  if (![object.confirmedFacts, object.hypotheses, object.unknowns, object.cautions].every(Array.isArray)) {
    throw new LlmError('invalid_output', 'AI出力の根拠整理が正しい配列形式ではありませんでした。');
  }
  return { confirmedFacts, hypotheses, unknowns, cautions };
}
...
- confirmedFactsには今回の入力または蓄積データに明記された事実だけを書く。推測はhypotheses、未確認はunknownsへ分ける。
...
出力するJSONのキーと内容:
{
  ...
  "grounding": {
    "confirmedFacts": ["人脈カード・追加メモ・履歴に明記された事実のみ"],
    "hypotheses": ["可能性はあるが未確認の見立て"],
    "unknowns": ["今回確認すべき未確認事項"],
    "cautions": ["押しすぎや紹介者の信用消費を防ぐ注意"]
  }
}`;
    ...
    return generateValidated(prompt, (raw): PreMeetingNavigation => {
      ...
      return {
        ...
        grounding: requireGrounding(raw),
      };
    });
  },
...
4. 予定前の仮説や人脈カード
5. 過去履歴
後メモ入力と予定前の仮説が矛盾する場合は、後メモ入力を採用すること。
すでに終わった面談・情報交換・送信をnextActionへ戻さないこと。過去日時を次の予定として出さないこと。
分類は会話後入力に明記された根拠がある場合だけ変更すること。紹介・人脈の話が出ていなければ紹介元候補/紹介先候補を追加しない。前向きな発言がなければ「可能性が高い」と断定しない。
...
出力するJSONのキーと内容:
{
  ...
  "grounding": {
    "confirmedFacts": ["質問回答・自由メモに明記された事実のみ。必ずJSON文字列配列"],
    "hypotheses": ["会話から考えられるが未確認の見立て。必ずJSON文字列配列"],
    "unknowns": ["次回確認すべき未確認事項。なければ空配列"],
    "cautions": ["温度感の過大評価や完了済み行動の再提案を防ぐ注意。必ずJSON文字列配列"]
  }
}`;
...
話者を厳密に分けること。「受信文チェック」では対象文面は相手の発言であり、replyDraftは営業担当者から相手へ返す文章にする。
「忙しい」「今は必要ない」「またタイミングが合えば」などの断り・保留では、追加質問をせず、深追いせず、短く関係を残す返信にする。翌日など早すぎる追客を提案しない。
断り・保留の場合はnextQuestionを必ず「追加質問なし」とし、nextActionは「短く返信してこちらからは追わない」、nextContactは最低でも2週間後か「相手からの連絡待ち」にする。replyDraftで相手の断り文を話者を変えずに繰り返さない。
断り・保留のreplyDraftは「承知しました。ご返信ありがとうございます。また必要な際はお声がけください。」程度で終え、面談・再連絡・相談の機会を求めない。
...
出力するJSONのキーと内容:
{
  ...
  "nextQuestion": "次に聞くべき質問（1文）。断り・保留の文面なら質問を作らず、必ず『追加質問なし』",
  "questionPurpose": "その質問の目的（1文）",
  "replyDraft": "そのまま送れる自然な返信文面（2〜3文）",
  "categoryUpdate": "分類の更新案（1〜2文）",
  "nextAction": "次の具体アクション（1文）。断り・保留なら『短く返信してこちらからは追わない』",
  "nextContact": "次回連絡の目安。断り・保留なら『相手からの連絡待ち』または最低2週間後。通常時のみ明日/3日後/1週間後",
  "caution": "注意点（1〜2文）",
  "feedbackGood": "今回のやり取りの良い点（1文）",
  "feedbackImprove": "改善点（1文）",
  "grounding": {
    "confirmedFacts": ["対象文面に明記された事実のみ"],
    "hypotheses": ["文面から考えられるが断定できない見立て"],
    "unknowns": ["まだ確認できないこと"],
    "cautions": ["話者取り違え・深追い・断定を防ぐ注意"]
  }
}`;
